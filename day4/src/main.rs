...
fn main() -> std::io::Result<()> {
    ...
    //println!("{} {}", *counts_vec[0].0, *counts_vec[0].1);
    let mut min_occur: Vec<(u32, u32, usize)> = Vec::new();
    for (guard_id, minute_vec) in &guard_minutes {
        //for min in (0..60) {
        //    println!("{} : {}", min, guard_minutes[counts_vec[0].0].iter().filter(|&n| *n == min).count());
        //}
        for min in (0..60) {
            //println!("{} {}", min, minute_vec.iter().filter(|&n| *n == min).count());
            min_occur.push((*guard_id, min, minute_vec.iter().filter(|&n| *n == min).count()));
        }
    }
    min_occur.sort_by(|a, b| b.2.cmp(&a.2));
    println!("{:?}", min_occur[0]);
